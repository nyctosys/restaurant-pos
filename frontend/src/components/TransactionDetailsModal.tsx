...
export default function TransactionDetailsModal({
  ...
  const [actionLoading, setActionLoading] = useState<
    "rollback" | "print" | "archive" | "unarchive" | "delete" | null
  >(null);
  ...
  const handleRollback = async () => {
    const confirmed = await showConfirm({
      ...
      message:
        "Are you certain you want to rollback this transaction? This will refund the amount and return items to inventory.",
        ...
        <div className="p-5 lg:p-6 overflow-y-auto flex-1 min-h-0">
          ...
          {loading || !details ? (
            ...
            <div className="space-y-6">
                  ...
                  {details.order_type === "delivery" &&
                    details.order_snapshot && (
                      <div className="text-sm text-soot-800 space-y-1">
                        ...
                        {!details.order_snapshot.address &&
                          details.order_snapshot.nearest_landmark && (
                            <p>
                              <span className="text-soot-500">
                                Nearest landmark:{" "}
                              </span>
                              {details.order_snapshot.nearest_landmark}
                            </p>
              ...
              {/* Items */}
              <div>
                <h4 className="text-sm font-bold text-soot-900 mb-3 border-b border-soot-100 pb-2">
                  Items
                </h4>
                    ...
                    >
                      <div className="flex-1">
                        <p className="font-medium text-soot-800">
                          {item.product_title}
                          {item.variant_sku_suffix && (
                            <span className="ml-2 text-xs font-bold bg-soot-100 px-1.5 py-0.5 rounded text-soot-600">
                              {item.variant_sku_suffix}
                            </span>
                          )}
                        </p>
                        <p className="text-soot-500 text-xs mt-0.5">
                          {formatCurrency(item.unit_price)} &times;{" "}
                          {item.quantity}
                        </p>
                      </div>
              ...
              <div className="border-t border-soot-200 pt-4 flex flex-col items-end text-sm">
                {(details.service_charge || 0) > 0 && (
                  <div className="flex justify-between w-48 mb-1">
                    ...
                    <span className="font-medium text-soot-800">
                      {formatCurrency(details.service_charge || 0)}
                    </span>
                  </div>
                )}
                {(details.delivery_charge || 0) > 0 && (
                  <div className="flex justify-between w-48 mb-1">
                    ...
                    <span className="font-medium text-soot-800">
                      {formatCurrency(details.delivery_charge || 0)}
                    </span>
                  </div>
                )}
                <div className="flex justify-between w-48 mb-1">
                  ...
                  <span className="font-medium text-soot-800">
                    {formatCurrency(details.tax_amount)}
                  </span>
                </div>
