...
export default function AuthGuard() {
  const [status, setStatus] = useState<
    "loading" | "needs_setup" | "needs_login" | "authenticated"
  >("loading");
    ...
    const checkAuthStatus = async () => {
      try {
        ...
        if (!data?.initialized) {
          log.info(
            "AuthGuard",
            "System not initialized → redirecting to /setup",
          );
        ...
        log.error(
          "AuthGuard",
          "Failed to verify system status; backend may be unavailable.",
          error,
        );
        setStatus("loading");
      }
  ...
  if (status === "loading") {
    return (
      <div className="flex h-full items-center justify-center text-soot-400">
