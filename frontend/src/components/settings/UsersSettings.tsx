        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative group">
            ...
            <select
              ...
              className="pl-10 pr-4 py-2 bg-white border border-soot-200 rounded-lg text-sm font-medium text-soot-700 focus:ring-2 focus:ring-brand-500 focus:outline-none appearance-none cursor-pointer hover:border-soot-300 transition-colors shadow-sm min-w-[160px]"
            >
      ...
      <div className="bg-white rounded-xl border border-soot-200 overflow-hidden">
        {loading ? (
          ...
          <table className="w-full text-left">
            <thead className="bg-soot-50 border-b border-soot-200">
              <tr>
            ...
            <tbody className="divide-y divide-soot-100">
              {filteredUsers.map((user) => (
                <tr key={user.id} className={`hover:bg-soot-50 transition-colors ${user.archived_at ? 'bg-soot-50/70 opacity-90' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {user.archived_at ? (
                        <>
                          <button onClick={() => handleRestoreUser(user)} className="p-1.5 text-soot-400 hover:text-emerald-600 hover:bg-emerald-50 rounded transition-colors" title="Restore">
