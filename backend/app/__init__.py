        ...
        for i in range(retries):
            try:
                ...
                try:
                    ...
                    db.session.rollback()
                try:
                    db.session.execute(text("ALTER TABLE sales ADD COLUMN kitchen_status VARCHAR(20) DEFAULT 'placed'"))
                    db.session.commit()
                    print("Added 'kitchen_status' column to 'sales' table.")
                except (OperationalError, ProgrammingError):
                    db.session.rollback()
                try:
                    db.session.execute(text("ALTER TABLE sales ADD COLUMN modifications JSON DEFAULT '[]'"))
                    db.session.commit()
                    print("Added 'modifications' column to 'sales' table.")
                except (OperationalError, ProgrammingError):
                    db.session.rollback()
                break
